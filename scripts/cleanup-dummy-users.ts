...
async function cleanupDummyUsers() {
  ...
  try {
    ...
    const dummyUsers = await prisma.user.findMany({
      where: {
        OR: [
          ...
          // Match by email
          ...DUMMY_USER_PATTERNS.filter((p) => p.includes("@")).map(
            (pattern) => ({
    ...
    for (const user of dummyUsers) {
      ...
      console.log(
        `    Sessions: ${user._count.sessions}, Accounts: ${user._count.accounts}`,
      );
    ...
    const usersToDelete = dummyUsers.filter(
      (user) =>
        !user.emailVerified &&
    ...
    if (usersToDelete.length === 0) {
      console.log(
        "⚠️  No clearly identifiable dummy users to delete. Manual review recommended.",
      );
      return;
    }
    ...
    console.log(
      "- System now only works with authenticated users (lvadlamudi, etc.)",
    );
    console.log(
      "- Users must log in through OAuth to become available interviewers",
    );
  } catch (error) {
