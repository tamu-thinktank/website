...
const RatingDropdown = ({ ratingFilter, setRatingFilter }: { ratingFilter: string; setRatingFilter: (value: string) => void }) => {
    return (
        <DropdownMenu>
...
const TeamDropdown = ({ teamFilter, setTeamFilter }: { teamFilter: string; setTeamFilter: (value: string) => void }) => {
    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <button
...
export default function ApplicantsTable() {
    ...
    const filteredApplicants = applicants.filter((applicant) => {
        return (
            (!searchName || applicant.name.toLowerCase().includes(searchName.toLowerCase())) &&
    ...
    return (
        <div className="max-h-[95%] overflow-auto flex flex-col items-center">
            <div className="sticky top-0 z-10 flex gap-4 bg-background py-4">
                <Input
                    placeholder="Search by Name"
                    className="w-60 border-white rounded-full text-white border-[0.75px]"
                    value={searchName}
                ...
                <Table>
                    <TableHeader>
                        <TableRow className="border-b border-white bg-zinc-700 border-opacity-25">
                            ...
                            <TableHead className="text-center text-white">Submission Date</TableHead>
                            <TableHead></TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {filteredApplicants.length ? (
                            filteredApplicants.map((applicant, index) => (
                                <TableRow key={index} className="border-b border-white border-opacity-25">
                                    <TableCell className="text-center">{applicant.name}</TableCell>
                                    <TableCell className="text-center">{applicant.uin}</TableCell>
                                    <TableCell className="text-center">{applicant.submissionDate}</TableCell>
