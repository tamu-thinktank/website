...
export const IntervieweesPage: React.FC = () => {
  ...
  const handleTransfer = () => {
    const lockedApplicants = filteredApplicants.filter(
    ...
    const isConfirmed = window.confirm(
      "Are you sure you want to transfer the locked applicants?",
    );
    if (isConfirmed) {
      setMembers((prevMembers) => [...prevMembers, ...lockedApplicants]);
      const updatedApplicants = applicantData.filter(
        (applicant) =>
          !lockedApplicants.some((locked) => locked.id === applicant.id),
      );
      setApplicantData(updatedApplicants);
      alert(
        `Transfer complete. ${lockedApplicants.length} applicant(s) transferred successfully.`,
      );
    }
  };
      ...
      <div className="mt-1 flex w-full flex-col items-center overflow-hidden px-20 pb-96 pt-11  max-md:max-w-full max-md:px-5 max-md:pb-24">
        <div className="mb-0 flex w-full max-w-[1537px] flex-col max-md:mb-2.5 max-md:max-w-full">
          ...
          <div className="ml-7 mt-8 flex w-auto max-w-full items-center justify-start gap-10 self-stretch px-0 py-0 text-sm tracking-wide text-neutral-200 max-md:flex-col">
            ...
            />
            <button
              className="rounded-[48px] border border-solid bg-stone-600 px-6 py-3 text-white transition-colors hover:bg-stone-500"
