...
export const CacheKeys = {
  ...
  busyTimes: (interviewerId: string, date: string) =>
    `scheduler:busy:${interviewerId}:${date}`,
  ...
  interviewsRange: (startDate: string, endDate: string) =>
    `scheduler:interviews:${startDate}:${endDate}`,

  // Pre-computed availability matrix for an interviewer and date
  availability: (interviewerId: string, date: string) =>
    `scheduler:availability:${interviewerId}:${date}`,
  ...
  priorityTeams: (interviewerId: string) =>
    `scheduler:priority:${interviewerId}`,
...
export class SchedulerCache {
   ...
   */
  static async setAllInterviewers(
    data: unknown[],
    ttl: number = CacheTTL.SHORT,
  ) {
   ...
   */
  static async setBusyTimes(
    interviewerId: string,
    date: string,
    busyTimes: unknown[],
    ttl: number = CacheTTL.SHORT,
  ) {
    return redis.set(CacheKeys.busyTimes(interviewerId, date), busyTimes, {
      ex: ttl,
    });
  }
  ...
  static async setAvailability(
    ...
    return redis.set(
      CacheKeys.availability(interviewerId, date),
      availability,
      { ex: ttl },
    );
  }
   ...
   */
  static async setPriorityTeams(
    interviewerId: string,
    teams: unknown[],
    ttl: number = CacheTTL.MEDIUM,
  ) {
    return redis.set(CacheKeys.priorityTeams(interviewerId), teams, {
      ex: ttl,
    });
  }
   ...
   */
  static async setAutoSchedulerResult(
    applicantId: string,
    result: unknown,
    ttl: number = CacheTTL.MEDIUM,
  ) {
  ...
  static async getApplicants(status = "all") {
  ...
  static async getStats() {
    try {
      ...
      const stats = {
        ...
        keysByType: {
          interviewers: allKeys.filter((k) => k.includes(":interviewer:"))
            .length,
          busyTimes: allKeys.filter((k) => k.includes(":busy:")).length,
          interviews: allKeys.filter((k) => k.includes(":interviews:")).length,
          availability: allKeys.filter((k) => k.includes(":availability:"))
            .length,
